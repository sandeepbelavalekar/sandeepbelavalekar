...
public class JwtAuthorizationFilter extends BasicAuthenticationFilter {
    ...
    private UsernamePasswordAuthenticationToken getAuthentication1(HttpServletRequest request) {
        String token = request.getHeader(SecurityConstants.TOKEN_HEADER);
        
            try {
                

                String username = "sandeepb";//parsedToken.getBody().getSubject();
                Collection<SimpleGrantedAuthority> authorities = new ArrayList<>();
                SimpleGrantedAuthority s =new SimpleGrantedAuthority("Admin");
                authorities.add(s);
				/*
				 * Collection<? extends GrantedAuthority> authorities = ((List<?>)
				 * parsedToken.getBody() .get("rol")).stream() .map(authority -> new
				 * SimpleGrantedAuthority((String) authority)) .collect(Collectors.toList());
				 */
